...
try {
    ...
    admin_log_action($conn, 'Added a library book to the "' . (string)($data['category_key'] ?? '') . '" category — Title (EN): "' . (string)($data['title_en'] ?? '') . '"; Title (AR): "' . (string)($data['title_ar'] ?? '') . '"; Series (EN): ' . admin_action_value($data['series_en'] ?? '') . '; Series (AR): ' . admin_action_value($data['series_ar'] ?? '') . '; Shown on the website: ' . (empty($data['is_public']) ? 'No' : 'Yes') . '.', ADMIN_ACTION_CATEGORY_LIBRARY);
