...
try {
    ...
    $bookingCheckSql = "SELECT booking_id, auth_id FROM event_bookings WHERE booking_id = ?";
    ...
    $parentSql = "SELECT parent_id FROM event_booking_parents_linker WHERE booking_id = ?";
    ...
    $stmt->close();
    $studentSql = "SELECT student_id FROM event_booking_students_linker WHERE booking_id = ?";
    ...
    try {
        ...
        admin_log_action($conn, 'Deleted the event booking #' . $bookingId . ' and all of its related data.', ADMIN_ACTION_CATEGORY_EVENT_BOOKINGS);
        echo json_encode([
            'success' => true,
