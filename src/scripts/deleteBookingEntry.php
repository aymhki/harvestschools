...
try {
    ...
    if (!isset($data['bookingId'])) {
        throw new Exception("Missing required parameter: bookingId", 400);
    }
    $bookingId = (int)$data['bookingId'];
    $sessionId = $_COOKIE['harvest_schools_admin_session_id'];
    $conn = new mysqli($servername, $username, $password, $dbname);
    ...
    // Get booking auth username for session deletion
    ...
    // Get parent IDs linked to this booking for later deletion
    ...
    // Get student IDs linked to this booking for later deletion
    ...
    try {
        ...
        // 7. Delete the main booking record
        $stmt = $conn->prepare("DELETE FROM bookings WHERE booking_id = ?");
        $stmt->bind_param("i", $bookingId);
        if (!$stmt->execute()) {
            throw new Exception("Failed to delete booking: " . $stmt->error);
