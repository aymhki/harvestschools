...
try {
    ...
    if ($returnAll) {
        ...
        $res = $conn->query(
            "SELECT DISTINCT s.student_id, s.name, s.grade, s.school_division
             ...
             JOIN event_booking_students_linker sl ON sl.student_id = s.student_id
             ORDER BY s.name ASC
             LIMIT 5000"
        );
        if ($res) {
    ...
    $sql = "
        ...
        FROM event_booking_students s
        LEFT JOIN event_booking_students_search_index i ON i.student_id = s.student_id
    ...
    if (count($candidates) < $maxOut) {
        $stmt = $conn->prepare("SELECT student_id, name, grade, school_division FROM event_booking_students LIMIT 2000");
        $stmt->execute();
        $res = $stmt->get_result();
        $stmt->close();
        while ($r = $res->fetch_assoc()) {
