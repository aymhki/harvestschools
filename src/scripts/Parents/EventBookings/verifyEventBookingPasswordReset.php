...
require_once 'EventBookingAuthHelpers.php';
set_cors_headers();
...
try {
    ...
    $stmt = $conn->prepare(
        "SELECT auth_id, attempts FROM event_booking_password_reset_challenges
    ...
    if (pwreset_attempts_exceeded($row['attempts'], $maxAttempts)) {
        $stmt = $conn->prepare("DELETE FROM event_booking_password_reset_challenges WHERE id = ?");
        $stmt->bind_param("s", $resetHash);
        $stmt->execute();
        $stmt->close();
    ...
    if (!$ok) {
        $stmt = $conn->prepare("UPDATE event_booking_password_reset_challenges SET attempts = attempts + 1 WHERE id = ?");
        $stmt->bind_param("s", $resetHash);
        $stmt->execute();
        $stmt->close();
    ...
    $stmt = $conn->prepare(
        "UPDATE event_booking_auth_credentials
