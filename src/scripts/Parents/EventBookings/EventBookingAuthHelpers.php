...
function gb_config($key = null) {
    ...
    if ($config === null) {
        $config = [
            ...
            'mail_from_name'              => 'Harvest Schools Events',
            'admin_notification_email'    => configured_email('booking-notifications'),
        ];

        $docRoot      = rtrim($_SERVER['DOCUMENT_ROOT'] ?? '', '/\\');
        $overridePath = $docRoot !== '' ? dirname($docRoot) . '/configs/EventBookingConfig.php' : '';
...
function gb_reset_context($lang = 'en') {
    ...
    return [
        'codes_table'    => 'event_booking_password_reset_codes',
        'send_log_table' => 'event_booking_password_reset_send_log',
        ...
        'subject'        => $ar ? 'رمز إعادة تعيين كلمة مرور الحجز' : 'Event booking password reset code',
...
function gb_parent_emails_for_auth($conn, $authId) {
    $sql = "SELECT DISTINCT p.email
            FROM event_booking_parents p
            JOIN event_booking_parents_linker pl ON p.parent_id = pl.parent_id
            JOIN event_bookings b ON b.booking_id = pl.booking_id
...
function gb_send_username_email($to, $username, $lang = 'en') {
    $ar = ($lang === 'ar');
    $subject = $ar ? 'اسم المستخدم الخاص بحجزك' : 'Your event booking username';
    $safeUsername = preg_replace('/[\r\n]+/', ' ', (string)$username);
    $body = ($ar
        ? "لقد طلب أحدهم استرجاع اسم المستخدم الخاص بالحجز المرتبط بهذا البريد الإلكتروني.\r\n\r\nاسم المستخدم الخاص بك هو:\r\n\r\n    {$safeUsername}\r\n\r\nإذا لم تطلب ذلك، يمكنك تجاهل هذه الرسالة."
        : "Someone requested the event booking username associated with this email address.\r\n\r\nYour username is:\r\n\r\n    {$safeUsername}\r\n\r\nIf you did not request this, you can safely ignore this message.");
...
function gb_send_admin_notice($conn, $authId, $username, $reason) {
    ...
    if ($authId !== null) {
        $stmt = $conn->prepare(
            "SELECT b.booking_id, b.booking_date, b.status
             FROM event_bookings b WHERE b.auth_id = ?"
        );
        $stmt->bind_param("i", $authId);
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    $subject = 'Event booking recovery assistance needed';
    $body = "A parent could not self-serve an event booking recovery and has no email on file.\r\n\r\n"
        . "Reason: {$reason}\r\n"
...
function gb_request_throttle($conn, $ownerKey) {
    ...
    $stmt = $conn->prepare(
        "SELECT COUNT(*) AS c FROM event_booking_password_reset_send_log
...
function gb_recovery_throttle($conn, $action) {
    ...
    $stmt = $conn->prepare(
        "SELECT COUNT(*) AS c FROM event_booking_recovery_throttle
    ...
    $stmt = $conn->prepare(
        "INSERT INTO event_booking_recovery_throttle (client_key, action) VALUES (?, ?)"
    );
    $stmt->bind_param("ss", $clientKey, $action);
    $stmt->execute();
    $stmt->close();

    $conn->query("DELETE FROM event_booking_recovery_throttle WHERE created_at < (NOW() - INTERVAL 1 DAY)");
...
function gb_upsert_student_search_index($conn, $studentId, $name, $grade, $division) {
    ...
    $stmt = $conn->prepare(
        "INSERT INTO event_booking_students_search_index
