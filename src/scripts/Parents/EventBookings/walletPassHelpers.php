...
require_once __DIR__ . '/EventCeremonyHelpers.php';
...
function wallet_booking_summary(mysqli $conn, $bookingId) {
    ...
    $sql = "SELECT b.booking_id, b.status, b.booking_date, b.booking_time, ac.username, ac.password_hash
            ...
            JOIN event_booking_auth_credentials ac ON b.auth_id = ac.auth_id
            WHERE b.booking_id = ?";
    ...
    if ($stmt) {
        ...
        if ($bookingRow) {
            ...
            $extrasSql = "SELECT extra_id, cd_count, additional_attendees, payment_status
                          FROM event_booking_extras WHERE booking_id = ?";
            ...
            $summary = [
                'ceremony'     => event_meta_details($conn),
...
function wallet_confirmation_url(array $summary) {
    return rtrim((string)wallet_config('confirmation_base_url'), '/')
        . '/events/event-booking-confirmation/?bookingId=' . rawurlencode((string)$summary['booking_id'])
...
function wallet_pass_json(array $summary) {
    ...
    $eventStart = (string)event_meta_iso_datetime($ceremony);
    ...
    $pass = [
        ...
        'serialNumber'         => 'event-' . $summary['booking_id'],
