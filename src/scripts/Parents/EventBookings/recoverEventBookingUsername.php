    ...
    foreach (array_unique($bookingIds) as $bid) {
        ...
        $stmt = $conn->prepare(
            "SELECT ac.username
             ...
             JOIN event_booking_auth_credentials ac ON ac.auth_id = b.auth_id
             WHERE b.booking_id = ?"
        );
    ...
    if ($method === 'student') {
        ...
        $stmt = $conn->prepare(
            "SELECT sl.booking_id
             FROM event_booking_students_linker sl
             WHERE sl.student_id = ?"
        );
        ...
        $stmt = $conn->prepare(
            "SELECT pl.booking_id, p.email
             ...
             JOIN event_booking_parents_linker pl ON pl.parent_id = p.parent_id
