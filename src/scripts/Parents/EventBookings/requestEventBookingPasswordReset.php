...
try {
    ...
    $stmt = $conn->prepare(
        "DELETE FROM event_booking_password_reset_challenges WHERE auth_id = ? OR expires_at < NOW()"
    );
    ...
    $stmt = $conn->prepare(
        "INSERT INTO event_booking_password_reset_challenges (id, auth_id, expires_at)
