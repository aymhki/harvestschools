...
function graduation_ceremony_details(mysqli $conn) {
    $details = [
        'ceremonyDate'      => null,
        'ceremonyTime'      => null,
        'timeZone'          => null,
    ...
    $result = $conn->query(
        "SELECT ceremony_date, ceremony_time, time_zone, location_name, location_address,
         ...
         FROM graduation_ceremony_details WHERE id = 1"
    );
...
function graduation_ceremony_is_valid_time_zone($timeZone) {
    return is_string($timeZone) && $timeZone !== ''
        && in_array($timeZone, DateTimeZone::listIdentifiers(), true);
}


function graduation_ceremony_iso_datetime(array $details) {
    $isoDateTime = null;

    if (!empty($details['ceremonyDate'])) {
        $time = !empty($details['ceremonyTime']) ? $details['ceremonyTime'] : '00:00:00';

        $timeZone = graduation_ceremony_is_valid_time_zone($details['timeZone'] ?? null)
            ? $details['timeZone']
...
function graduation_ceremony_location_label(array $details) {
