...
try {
    ...
    $stmt = $conn->prepare("SELECT webauthn_challenge FROM alumni_sessions WHERE id = ?");
    ...
    $stmt = $conn->prepare("UPDATE alumni_sessions SET webauthn_challenge = NULL WHERE id = ?");
    $stmt->bind_param("s", $tokenHash);
    $stmt->execute();
    $stmt->close();
    ...
    $stmt = $conn->prepare("SELECT id FROM alumni_passkeys WHERE credential_id = ?");
