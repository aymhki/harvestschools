...
//     const [isMobile, setIsMobile] = useState(window.innerWidth < 768);
//     const [finalTableData, setFinalTableData] = useState(tableData); // This is the data that will be displayed after filtering and sorting and hiding columns
//     const [rowMapping, setRowMapping] = useState([]); // Maps displayed rows to original indices
//     const contentAnimation = useSpring({
//         opacity: isAccordionOpen ? 1 : 0,
//         transform: isAccordionOpen ? 'translateY(0)' : 'translateY(-100%)',
//         config: { duration: 300 },
...
//     });
//
//     // Create sorted data with original indices
//     const sortedDataWithIndices = useMemo(() => {
//         if (sortConfig.column === null) {
//             return tableData ? tableData.map((row, index) => ({ row, originalIndex: index })) : [];
//         }
//         if (!tableData) {
//             return [];
//         }
...
//     }, [tableData, sortConfig, tableHeader]);
//
//     // Extract just the data for display
//     const sortedData = useMemo(() => {
...
//         }
//
//         // Create a mapping of displayed row indices to original indices
//         const newRowMapping = filteredDataWithIndices.map(item => item.originalIndex);
//         setRowMapping(newRowMapping);
//
//         // Filter out hidden columns
//         const hiddenColumnsData = filteredDataWithIndices.map(item =>
//             item.row.filter((cell, index) => !hiddenColumns.has(sortedData[0][index]))
//         );
//
//         setFinalTableData(hiddenColumnsData);
...
function Table({
    ...
    const sortedDataWithIndices = useMemo(() => {
        ...
        // If no sorting is applied, return unsorted data
    ...
    // Updated requestSort function with immediate state update
    const requestSort = (columnIndex) => {
        let direction = 'ascending';
    ...
    const updateFinalTableData = useCallback(() => {
        ...
        if (filterableColumns && filterableColumns.length > 0) {
            for (let i = 0; i < tableData[0].length; i++) {
                ...
                if (filterableColumns.includes(columnName) && filterUniqueValuesDict[columnName]) {
                    ...
                    filteredDataWithIndices = filteredDataWithIndices.filter((item, rowIndex) => {
                        if (rowIndex === 0 || (tableHeader && rowIndex === 1)) {
                            return true; // Always keep header rows
                        }
                        ...
                        if (valueIndex === -1) {
                            return true; // Keep rows with values not in the filter list
                        }

                        return columnFilter.checked[valueIndex]; // Keep rows with checked values
                    });
                }
            }
        }
        ...
        const filteredData = filteredDataWithIndices.map(item =>
            item.row.filter((cell, colIndex) =>
                // Ensure we're using the correct header row for filtering
    ...
    // Ensure table data updates whenever any relevant state changes
    useEffect(() => {
        updateFinalTableData();
    }, [hiddenColumns, sortedDataWithIndices, filterUniqueValuesDict, updateFinalTableData]);

    // Make sure sorting updates trigger immediately
    useEffect(() => {
        updateFinalTableData();
