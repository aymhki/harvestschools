...
const departmentsColIndex = 8;
const displayColIndex = 9;
const isPublicColIndex = 10;
const emailColIndex = 11;
const phoneColIndex = 12;
const hireDateColIndex = 13;
const notesColIndex = 14;
...
function StaffDirectoryManagement() {
    ...
    const buildFormFields = (rowData) => ([
        { id: sortOrderFormFieldId, type: 'number', name: 'sort_order', label: 'ID', required: false,
            value: rowData ? rowData[sortOrderColIndex] : '', defaultValue: rowData ? rowData[sortOrderColIndex] : '',
            minimumValue: 0, maximumValue: 100000, widthOfField: 3, labelOutside: true, labelOnTop: true,
            displayLabel: 'ID (also the order the table and the staff pages use)', httpName: 'staff-sort-order',
            alwaysEnglish: true },
        { id: employeeCodeFormFieldId, type: 'text', name: 'employee_code', label: 'Employee ID', required: false,
            value: rowData ? rowData[employeeCodeColIndex] : '', widthOfField: 3, labelOutside: true, labelOnTop: true,
            displayLabel: rowData ? 'Employee ID' : 'Employee ID (leave blank to generate the next one)',
            readOnlyField: !!rowData, httpName: 'staff-employee-code', dontLetTheBrowserSaveField: true },
        { id: isPublicFormFieldId, type: 'select', name: 'is_public', label: 'Shown Publicly', required: true,
            choices: ['Yes', 'No'], defaultValue: rowData ? rowData[isPublicColIndex] : 'No',
            value: rowData ? rowData[isPublicColIndex] : 'No', widthOfField: 3, labelOutside: true, labelOnTop: true,
            displayLabel: 'Shown Publicly', httpName: 'staff-is-public' },
        { id: nameEnFormFieldId, type: 'text', name: 'name_en', label: 'Name (EN)', required: true,
            errorMsg: 'Please enter the English name', value: rowData ? rowData[nameEnColIndex] : '',
            widthOfField: 2, labelOutside: true, labelOnTop: true, displayLabel: 'Name (EN)', httpName: 'staff-name-en',
            dontLetTheBrowserSaveField: true },
        { id: nameArFormFieldId, type: 'text', name: 'name_ar', label: 'Name (AR)', required: true,
            errorMsg: 'Please enter the Arabic name', value: rowData ? rowData[nameArColIndex] : '',
            widthOfField: 2, labelOutside: true, labelOnTop: true, displayLabel: 'Name (AR)', lang: 'ar',
            httpName: 'staff-name-ar', dontLetTheBrowserSaveField: true },
        { id: positionEnFormFieldId, type: 'text', name: 'position_en', label: 'Position (EN)', required: true,
            errorMsg: 'Please enter the English position', value: rowData ? rowData[positionEnColIndex] : '',
            widthOfField: 2, labelOutside: true, labelOnTop: true,
            displayLabel: 'Position (EN), separate more than one with a comma', httpName: 'staff-position-en' },
        { id: positionArFormFieldId, type: 'text', name: 'position_ar', label: 'Position (AR)', required: true,
            errorMsg: 'Please enter the Arabic position', value: rowData ? rowData[positionArColIndex] : '',
            widthOfField: 2, labelOutside: true, labelOnTop: true,
            displayLabel: 'Position (AR), separate more than one with a comma', lang: 'ar',
            httpName: 'staff-position-ar' },
        { id: subjectEnFormFieldId, type: 'text', name: 'subject_en', label: 'Subject (EN)', required: false,
            value: rowData ? rowData[subjectEnColIndex] : '', widthOfField: 2, labelOutside: true, labelOnTop: true,
            displayLabel: 'Subject (EN), separate more than one with a comma', httpName: 'staff-subject-en' },
        { id: subjectArFormFieldId, type: 'text', name: 'subject_ar', label: 'Subject (AR)', required: false,
            value: rowData ? rowData[subjectArColIndex] : '', widthOfField: 2, labelOutside: true, labelOnTop: true,
            displayLabel: 'Subject (AR), separate more than one with a comma', lang: 'ar',
            httpName: 'staff-subject-ar' },
        { id: departmentsFormFieldId, type: 'select', name: 'departments', label: 'Departments', required: true,
            multiple: true, large: true, choices: departmentNames,
            errorMsg: 'Please choose at least one department',
            defaultValue: rowData ? rowData[departmentsColIndex] : '',
            value: rowData ? rowData[departmentsColIndex] : '',
            widthOfField: 1, labelOutside: true, labelOnTop: true,
            displayLabel: 'Departments', httpName: 'staff-departments' },
        { id: displayFormFieldId, type: 'select', name: 'display_style', label: 'Display', required: true,
            choices: displayStyles.length ? displayStyles : ['Table Row'],
            defaultValue: rowData ? rowData[displayColIndex] : 'Table Row',
            value: rowData ? rowData[displayColIndex] : 'Table Row',
            widthOfField: 3, labelOutside: true, labelOnTop: true, displayLabel: 'Display', httpName: 'staff-display' },
        { id: emailFormFieldId, type: 'email', name: 'email', label: 'Email', required: false,
            value: rowData ? rowData[emailColIndex] : '', widthOfField: 3, labelOutside: true, labelOnTop: true,
            displayLabel: 'Email (never published)', httpName: 'staff-email', dontLetTheBrowserSaveField: true },
        { id: phoneFormFieldId, type: 'text', name: 'phone', label: 'Phone', required: false,
            value: rowData ? rowData[phoneColIndex] : '', widthOfField: 3, labelOutside: true, labelOnTop: true,
            displayLabel: 'Phone (never published)', httpName: 'staff-phone', dontLetTheBrowserSaveField: true },
        { id: hireDateFormFieldId, type: 'date', name: 'hire_date', label: 'Hire Date', required: false,
            value: rowData ? rowData[hireDateColIndex] : '', defaultValue: rowData ? rowData[hireDateColIndex] : '',
            widthOfField: 3, labelOutside: true, labelOnTop: true, displayLabel: 'Hire Date (never published)',
            httpName: 'staff-hire-date', alwaysEnglish: true },
        { id: notesFormFieldId, type: 'textarea', name: 'notes', label: 'Notes', required: false,
            value: rowData ? rowData[notesColIndex] : '', widthOfField: 1, labelOutside: true, labelOnTop: true,
            displayLabel: 'Notes (never published)', httpName: 'staff-notes' },
    ]);
    ...
    const handleEditorSubmit = async (formData) => {
        ...
        try {
            ...
            const payload = {
                ...
                employee_code: editorMode === 'edit'
                    ? employeesData[rowIndexToEdit][employeeCodeColIndex]
                    : (values[`field_${employeeCodeFormFieldId}`] || ''),
        ...
        <>
            ...
            <div className={"staff-directory-management-page"}>
                <Table tableData={employeesData}
                       ...
                       filterableColumns={['Departments', 'Display', 'Public']}
                ...
                <div className={"general-large-admin-action-modal-container"}>
                    ...
                    <div className={"general-large-admin-action-modal-content"}>
                        <p className={"general-large-admin-action-modal-content-note"}>
                            Note: Email, phone, hire date and notes are for the office only and are never sent to the
                            website. Someone who does more than one job goes in one row with both written in the
                            position field, separated by a comma. Anyone who serves the whole school, such as reception,
                            accounting, student affairs or HR, belongs in All Departments.
                        </p>
