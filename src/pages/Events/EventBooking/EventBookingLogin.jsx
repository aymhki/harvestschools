...
} from "../../../services/Parents/EventBookings/MainParentsEventBookingServices.jsx";
import {useTranslation} from "react-i18next";
import {headToEventBookingDashboardOnValidSession} from "../../../services/Parents/EventBookings/EventBookingNavigationServices.jsx";
...
function EventBookingLogin() {
    ...
    const handleBookingLogin = async (formData) => {
        ...
        try {
            const result = await validateEventBookingLogin(formData, usernameFieldId, passwordFieldId, navigate);
    ...
    const resetToFirstTimeMobileExperience = async () => {
        await deleteBiometricCredentials(EVENT_BOOKING_SESSION_NAME);
    ...
    const attemptBiometricLogin = async () => {
        ...
        try {
            ...
            const credentials = await getBiometricCredentials(EVENT_BOOKING_SESSION_NAME);
            ...
            const result = await validateEventBookingLoginWithCredentials(
    ...
    useEffect (() => {
        ...
        const autoSessionCheck = async () => {
            ...
            try {
                await headToEventBookingDashboardOnValidSession(navigate, setSubmittingLocal);
                ...
                const credentialsSaved = await hasSavedBiometricCredentials(EVENT_BOOKING_SESSION_NAME);
                if (!isMountedRef.current) {return;}
    ...
    const sendResetCode = async (resetToken, {silent = false} = {}) => {
        ...
        const result = await requestEventBookingResetEmailCode(resetToken);
        if (!isMountedRef.current) { return; }
        ...
        if (result?.success) {
            setPwResetError(null);
            if (!silent) { setPwResetStatus(t("events-pages.event-booking-pages.login-page.code-resent")); }
        } else if (!silent || result?.code === 429) {
            setPwResetStatus(null);
            setPwResetError(result?.message || t("events-pages.event-booking-pages.login-page.code-send-failed"));
        }
    };

    const handleForgotPassword = async (formData) => {
        const username = (Object.fromEntries(formData.entries())[`field_${forgotPwUsernameFieldId}`] || '').trim();
        const result = await requestEventBookingPasswordReset(username);
    ...
    const handleResetPassword = async (formData) => {
        ...
        if (newPassword !== confirmPassword) { throw new Error(t("events-pages.event-booking-pages.login-page.confirm-password-error")); }

        const result = await completeEventBookingPasswordReset(pwResetState.resetToken, code, newPassword, pwResetUsername);

        if (result && result.success) {
            setPrefillUsername(pwResetUsername);
            goToLoginView(result.message || t("events-pages.event-booking-pages.login-page.reset-success"));
            return true;
        }
        if (result && (result.code === 429 || /expired/i.test(result.message || ''))) {
            goToLoginView(result.code === 429
                ? t("events-pages.event-booking-pages.login-page.too-many-attempts")
                : t("events-pages.event-booking-pages.login-page.session-expired"));
            return true;
        }
    ...
    const ensureStudentsLoaded = async () => {
        ...
        const list = await listEventBookingStudents();
        if (!isMountedRef.current) { return; }
        setAllStudents(list);
    ...
    const revealUsernames = (result) => {
        ...
        if (result && result.success && !result.found) {
            ...
            setRecoverError(result.message || t("events-pages.event-booking-pages.login-page.username-not-found"));
            return true;
        }
    ...
    const handleRecoverByStudent = async (formData) => {
        ...
        if (!studentId) { throw new Error(t("events-pages.event-booking-pages.login-page.choose-child")); }
        const result = await recoverEventBookingUsername('student', {student_id: Number(studentId)});
    ...
    const handleRecoverByContact = async (formData) => {
        ...
        const result = await recoverEventBookingUsername(recoverMethod, {value});
        return revealUsernames(result);
    };
    ...
    const buildNewPasswordFields = () => ([
        {
            ...
            label: 'New Password', displayLabel: t("events-pages.event-booking-pages.login-page.new-password-field"),
            placeholder: t("events-pages.event-booking-pages.login-page.new-password-field"),
            errorMsg: t("events-pages.event-booking-pages.login-page.password-policy-error"),
        ...
        },
        {
            ...
            label: 'Confirm New Password', displayLabel: t("events-pages.event-booking-pages.login-page.confirm-new-password-field"),
            placeholder: t("events-pages.event-booking-pages.login-page.confirm-new-password-field"),
            errorMsg: t("events-pages.event-booking-pages.login-page.confirm-password-error"),
    ...
    const renderForgotPasswordScreen = () => (
        <>
            <p className={'booking-login-instructions'}>{t("events-pages.event-booking-pages.login-page.forgot-password-instructions")}</p>
            <Form key={'booking-forgot-password'} mailTo={''} sendPdf={false}
                  formTitle={'Event Booking Forgot Password'} noSuccessMessage={true} lang={'en'} captchaLength={1}
                  ...
                  differentSubmitButtonText={[t("events-pages.event-booking-pages.login-page.continue-btn"), t("events-pages.event-booking-pages.login-page.checking-btn")]}
                  fields={[{
                      id: forgotPwUsernameFieldId, type: 'text', name: 'username', httpName: 'username', required: true,
                      label: 'Username', displayLabel: t("events-pages.event-booking-pages.login-page.username-field"),
                      placeholder: t("events-pages.event-booking-pages.login-page.username-field"),
            ...
            <span className={'various-login-text-options'} onClick={() => goToLoginView(null)}>
                {t("events-pages.event-booking-pages.login-page.back-to-login")}
            </span>
        </>
    );
    ...
    const renderResetPasswordScreen = () => (
        <>
            <p className={'booking-login-instructions'}>
                {t("events-pages.event-booking-pages.login-page.reset-instructions", {emails: (pwResetState?.maskedEmails || []).join(', ')})}
            </p>
            <Form key={'booking-reset-password'} mailTo={''} sendPdf={false}
                  formTitle={'Event Booking Reset Password'} noSuccessMessage={true} lang={'en'} captchaLength={1}
                  noInputFieldsCache={true} noCaptcha={true} easySimpleCaptcha={true}
                  ...
                  hasDifferentSubmitButtonText={true}
                  differentSubmitButtonText={[t("events-pages.event-booking-pages.login-page.reset-submit"), t("events-pages.event-booking-pages.login-page.resetting-btn")]}
                  fields={[
                      ...buildNewPasswordFields(),
                      {
                          id: resetCodeFieldId, type: 'text', name: 'reset-code', httpName: 'one-time-code', required: true,
                          label: 'Verification Code', displayLabel: t("events-pages.event-booking-pages.login-page.verification-code-field"),
                          placeholder: '000000', errorMsg: t("events-pages.event-booking-pages.login-page.verification-code-error"),
            ...
            <div className={'booking-login-recovery-actions'}>
                <span className={'various-login-text-options'} onClick={() => {
                    ...
                    {resendIn > 0
                        ? t("events-pages.event-booking-pages.login-page.resend-in", {seconds: resendIn})
                        : t("events-pages.event-booking-pages.login-page.resend-code")}
                </span>
    ...
    const renderRecoveredUsernames = () => {
        ...
        return (
            <div className={'booking-login-username-results'} role={'status'}>
                <p>{t("events-pages.event-booking-pages.login-page.username-found")}</p>
                <ul>
                    {recoveredUsernames.map((u) => (
                        <li key={u}>
                            ...
                            <span className={'various-login-text-options'} onClick={() => applyRecoveredUsername(u)}>
                                {t("events-pages.event-booking-pages.login-page.use-this-username")}
                            </span>
                        </li>
                    ))}
                </ul>
        ...
        return (
            <>
                <Form key={'booking-recover-student'} mailTo={''} sendPdf={false}
                      formTitle={'Event Booking Find Username By Student'} noSuccessMessage={true} lang={'en'} captchaLength={1}
                      noInputFieldsCache={true} noCaptcha={true} easySimpleCaptcha={true}
                      noClearOption={true} centerSubmitButton={true} fullMarginField={true}
                      hasSetSubmittingLocal={true} setSubmittingLocal={setSubmittingLocal}
                      hasDifferentOnSubmitBehaviour={true} differentOnSubmitBehaviour={handleRecoverByStudent}
                      hasDifferentSubmitButtonText={true}
                      differentSubmitButtonText={[t("events-pages.event-booking-pages.login-page.find-username-btn"), t("events-pages.event-booking-pages.login-page.searching-btn")]}
                      fields={[{
                          id: studentFieldId, type: 'search-select', name: 'student', httpName: 'student', required: true,
                          label: 'Student', displayLabel: t("events-pages.event-booking-pages.login-page.student-field"),
                          placeholder: t("events-pages.event-booking-pages.login-page.student-search-placeholder"),
                          errorMsg: t("events-pages.event-booking-pages.login-page.choose-child"),
                          choices,
                          value: '', setValue: null, widthOfField: 1, labelOutside: true, labelOnTop: true,
                      }]}
                />
                <p className={'booking-login-hint'}>
                    {!studentsLoaded && t("events-pages.event-booking-pages.login-page.loading-students")}
                </p>
            </>
        );
    };
    ...
    const renderByContact = () => (
        <Form key={`booking-recover-${recoverMethod}`} mailTo={''} sendPdf={false}
              formTitle={'Event Booking Find Username By Contact'} noSuccessMessage={true} lang={'en'} captchaLength={1}
              noInputFieldsCache={true} noCaptcha={true} easySimpleCaptcha={true}
              noClearOption={true} centerSubmitButton={true} fullMarginField={true}
              hasSetSubmittingLocal={true} setSubmittingLocal={setSubmittingLocal}
              ...
              fields={[{
                  ...
                  displayLabel: recoverMethod === 'email'
                      ? t("events-pages.event-booking-pages.login-page.parent-email-field")
                      : t("events-pages.event-booking-pages.login-page.parent-phone-field"),
                  placeholder: recoverMethod === 'email'
                      ? t("events-pages.event-booking-pages.login-page.parent-email-field")
                      : t("events-pages.event-booking-pages.login-page.parent-phone-field"),
                  errorMsg: recoverMethod === 'email'
                      ? t("events-pages.event-booking-pages.login-page.email-error")
                      : t("events-pages.event-booking-pages.login-page.phone-error"),
                  value: '', setValue: null, widthOfField: 1, labelOutside: true, labelOnTop: true,
              }]}
        />
    );
    ...
    const renderForgotUsernameScreen = () => (
        <>
            <p className={'booking-login-instructions'}>{t("events-pages.event-booking-pages.login-page.forgot-username-instructions")}</p>
            <div className={'booking-login-method-switch'}>
                {['student', 'email', 'phone'].map((m) => (
                    <button key={m} type={'button'} className={m === recoverMethod ? 'current' : ''} aria-pressed={m === recoverMethod}
                        ...
                        {t(`events-pages.event-booking-pages.login-page.method-${m}`)}
                    </button>
                ))}
            </div>
    ...
    const renderBiometricScreen = () => (
        <div className={'booking-login-biometric-only'}>
            ...
            >
                {t("events-pages.event-booking-pages.login-page.biometric-button")}
            </button>
            <span
            ...
            >
                {t("events-pages.event-booking-pages.login-page.biometric-different-login")}
            </span>
        </div>
    );
            ...
            <div className={'booking-login-page'}>
                <div className={'booking-login-page-form-controller'}>
                    <div className={'booking-login-form-wrapper'}>
                        {loginMode !== 'checking' && (
                            <h2>
                                {t("events-pages.event-booking-pages.login-page.title")}
                            </h2>
                        )}
                        ...
                        <Form key={loginMode === 'recovery' ? 'booking-recovery-form' : 'booking-normal-form'}
                              mailTo={''}
                              sendPdf={false}
                              formTitle={t("events-pages.event-booking-pages.login-page.title")}
                              lang={'en'}
                              captchaLength={1}
                              ...
                              hasDifferentSubmitButtonText={true}
                              differentSubmitButtonText={[t("events-pages.event-booking-pages.login-page.login-btn"), t("events-pages.event-booking-pages.login-page.logging-in-btn")]}
                              noClearOption={true}
                                  ...
                                  [
                                      {
                                          ...
                                          label: 'Username',
                                          required: true,
                                          displayLabel: t("events-pages.event-booking-pages.login-page.username-field"),
                                          placeholder: t("events-pages.event-booking-pages.login-page.username-field"),
                                      ...
                                      },
                                      {
                                          ...
                                          label: 'Password',
                                          required: true,
                                          displayLabel: t("events-pages.event-booking-pages.login-page.password-field"),
                                          placeholder: t("events-pages.event-booking-pages.login-page.password-field"),
                        ...
                        <div className={'booking-login-recovery-links'}>
                            <span  className={'various-login-text-options'}
                                ...
                                {t("events-pages.event-booking-pages.login-page.forgot-username-link")}
                            </span>
                            <span  className={'various-login-text-options'}
                                    onClick={() => { setLoginNotice(null); setView('forgotPassword'); }}>
                                {t("events-pages.event-booking-pages.login-page.forgot-password-link")}
                            </span>
                        </div>
                        </>
                        )
...
export default EventBookingLogin;
