...
- [x] Add: Option to delete academic calendar year after creating it and naturally go back to the last available one.
...
- [x] Add: search field to table header elements with optional prop that will only show rows where any colum/row contains the query.
...
- [x] App opening normally should not navigate to the last opened page.
...
- [x] Add a narrow/wide option for video uploads similar to photo collages and update the css accordingly.
- [x] Editing Photo Collage or Videos should also allow editing the position/order.
    - [ ] Reordering the photos inside a collage still has no UI; the plan puts it in this item as a move up / move down pair.
...
- [ ] Swipe on videos should also work on touch/hover in mobile and the loading indicator should be a progress bar at the top because it currently hidden by play icon.
    - [x] Loading indicator is now a progress bar pinned to the top edge of the card instead of a spinner behind the play icon.
    - [ ] Touch swipe: not verifiable from here, use-gesture ignores synthetic pointer events, so it needs a real device.
- [x] Separate BOT_ON for whatsapp and messenger/Instagram.
- [x] Add a new column to the info system departments table available_to_chat_with where if it false then the bot should not show it in the chat with department menu for now student affairs, american department, and accounting are off. Bot should indicate that some department numbers are hidden because they are not available to chat with
- [x] Look into adding static content in the info system to edit things like the tuition fees disclaimer or the faqs or important notes used here and there in the bot like the minimum registration age requirement.
- [x] SiteVerify still flagged as sometimes not working on cloudflare.
- [x] Add: Analytics tab in the info system pulling from the Google Analytics Data API for the gtag on the main website, plus figures from the database such as the chatbot user sessions not yet cleared by the cron.
- [x] Add: Total employees (around 400) and total students (around 2000) into the school info at every layer where it fits, so the MCP tools, agents and assistants can answer with them.
- [ ] Add: "Import From CSV" next to the existing Export CSV on every admin table/list that has an add option, driven by an optional prop and a matching bulk add endpoint per domain. A wrongly formatted or incomplete CSV must come back with a descriptive error naming the offending row and column plus an example of correct usage for that endpoint's fields. The MCP bulk tools must not reuse the CSV endpoint, they take JSON, and only share the underlying add function.
    - [x] Library, Staff Directory, Academic Calendar events and Event Bookings.
    - [ ] Info System static content, now that that tab creates rows rather than only editing them.
    - [ ] Admin Users: needs a deliberate call first, an import there mints login credentials and assigns permission levels.
- [x] Add: Tables with action, edit or delete buttons should keep those columns sticky to the right by default, the same way the first column is sticky to the left, so only the columns in between scroll.
- [x] Fix: Adjust the widthOfField props across the borrowing system page forms so every line is filled, three of widthOfField 3 or two of widthOfField 2 or one of widthOfField 1, with no half line followed by a full line.
- [x] Add: A web app manifest for the admin portal and the main site so they install as desktop apps on Windows.
