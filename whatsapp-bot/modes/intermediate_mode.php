...
//        'departments' => [
//            'early' => [
//                'name' => ['en' => 'Pre-Play & Play', 'ar' => 'مرحلة ما قبل المدرسة'],
//                'sections' => [
//                    'early_stg' => [
...
//                            'stg_play' => ['name' => ['en' => 'Play', 'ar' => 'Playschool'], 'offered' => true, 'age' => ['en' => '2 years and 6 months', 'ar' => 'سنتان و 6 شهور'], 'fees' => 28750],
//                        ]
//                    ]
//                ]
//            ],
...
/**
 * Returns the complete multidimensional configuration for Harvest International Schools.
 * Includes all standard items, educational structures, FAQs, and WhatsApp contact lines.
 */
function getSchoolConfig()
{
    return [
        'ui' => [
            ...
            'faq_body' => ['en' => 'Select a question to view the answer:', 'ar' => 'اختر سؤالاً لعرض الإجابة:'],
            'back_btn' => ['en' => 'Main Menu', 'ar' => 'القائمة الرئيسية'],
            'nav_section' => ['en' => 'Navigation Options', 'ar' => 'خيارات التنقل'],
            'apply_btn' => ['en' => 'Apply Now', 'ar' => 'تقدم الآن'],
            'chat_title' => ['en' => 'Our Departments', 'ar' => 'أقسامنا المتاحة'],
            'chat_body' => ['en' => 'Please select the department you wish to chat with:', 'ar' => 'يرجى اختيار القسم الذي تود التحدث معه:'],
        ],
        'contact_departments' => [
            ...
            'american' => ['en' => 'American Department', 'ar' => 'الالقسم الامريكي', 'number' => '201028940675'],
        ],
        'main_options' => [
            ['id' => 'menu_stages', 'en' => 'Stages Offered', 'ar' => 'المراحل المتاحة'],
            ['id' => 'menu_age', 'en' => 'Minimum Age', 'ar' => 'سن القبول'],
            ...
            ['id' => 'menu_faqs', 'en' => 'FAQs', 'ar' => 'الأسئلة الشائعة'],
            ['id' => 'menu_careers', 'en' => 'Careers / Vacancies', 'ar' => 'الوظائف المتاحة'],
            ['id' => 'menu_chat', 'en' => 'Chat with a Department', 'ar' => 'التحدث مع احد الأقسام'],
        ],
        'static_content' => [
            'menu_disc' => [
                'en' => "*Discounts:*\n\n• *Siblings Discount:* 10% off tuition fees\n• *Staff Discount:* 40% off tuition fees\n\n_For combined discount cases, please confirm directly with our Accounting department to get an accurate quote._",
...
/**
 * Searches the structured configuration for a grade/stage data block matching the specific ID string.
 */
function findStageById($stageId)
{
    $config = getSchoolConfig();
...
/**
 * Generates formatted admission requirements text block for the specific educational stage.
 */
function getRequirementsForStage($stageId, $stageName, $lang)
{
    $medicalStages = ['stg_nat_kg1', 'stg_brit_fs1', 'stg_am_prek'];
...
/**
 * Builds the standard Navigation list section appended to all menus.
 * Contains Main Menu, Language Change, and Apply Now options.
 */
function getNavSection($lang, $contextId)
{
    $config = getSchoolConfig();
    $ui = $config['ui'];
    $toggleTitle = ($lang === 'en') ? 'تغيير للغة العربية' : 'Change to English';

    return [
        "title" => mb_substr($ui['nav_section'][$lang], 0, 24),
        "rows" => [
            ["id" => "main_menu", "title" => mb_substr($ui['back_btn'][$lang], 0, 24)],
            ["id" => "lang_toggle_" . $contextId, "title" => mb_substr($toggleTitle, 0, 24)],
            ["id" => "menu_apply", "title" => mb_substr($ui['apply_btn'][$lang], 0, 24)]
        ]
    ];
}

/**
 * Aggregates all data attributes for an educational grade into an all-inclusive report layout.
 */
function sendConsolidatedResult($to, $lang, $action, $stageId)
{
    $config = getSchoolConfig();
    $stageData = findStageById($stageId);
    if (!$stageData) return;
    ...
    if (!$stageData['offered']) {
        ...
        $reqs = getRequirementsForStage($stageId, $stageName, $lang);

        if ($lang === 'en') {
            $responseText = "★ *{$stageName} - Complete Information* ★\n\n";
            $responseText .= "• *Status:* Currently Offered\n";
            $responseText .= "• *Minimum Admission Age:* {$ageStr}\n";
            $responseText .= "• *Annual Tuition Fees:* {$feesStr} {$currency}{$disclaimer}\n\n";
            $responseText .= $reqs;
        } else {
            $responseText = "★ *معلومات كاملة عن: {$stageName}* ★\n\n";
            $responseText .= "• *الحالة:* متاحة حالياً للتسجيل\n";
            $responseText .= "• *الحد الأدنى لسن القبول:* {$ageStr}\n";
            $responseText .= "• *المصروفات الدراسية السنوية:* {$feesStr} {$currency}{$disclaimer}\n\n";
            $responseText .= $reqs;
        }
    }

    sendFinalTextWithMenuButton($to, $responseText, $lang, "res_{$action}_{$stageId}");
}

/**
 * High-level core execution router that intercepts user chat incoming flows for Intermediate Mode.
 */
function handleIntermediateMode($from, $message)
{
    $session = getSession($from);
    ...
    if ($type === 'interactive') {
        ...
        // Global Navigation Logic Interceptor for Language Swapping (Preserves Context State)
        if (strpos($replyId, 'lang_toggle_') === 0) {
            $context = substr($replyId, 12);
            $newLang = ($lang === 'en') ? 'ar' : 'en';
            createOrUpdateSession($from, $newLang, 'menu');

            $parts = explode('_', $context);
            if ($context === 'main') {
                sendMainMenuIntermediate($from, $newLang);
            } elseif ($context === 'faq') {
                sendFaqMenuIntermediate($from, $newLang);
            } elseif ($context === 'chat') {
                sendChatDepartmentMenuIntermediate($from, $newLang);
            } elseif ($parts[0] === 'dept') {
                sendDepartmentMenuIntermediate($from, $newLang, $parts[1]);
            } elseif ($parts[0] === 'sec') {
                sendSectionMenuIntermediate($from, $newLang, $parts[1], $parts[2]);
            } elseif ($parts[0] === 'stg') {
                sendStageMenuIntermediate($from, $newLang, $parts[1], $parts[2], $parts[3]);
            } elseif ($parts[0] === 'res') {
                $action = $parts[1];
                array_shift($parts);
                array_shift($parts);
                $stageId = implode('_', $parts);
                sendConsolidatedResult($from, $newLang, $action, $stageId);
            } else {
                sendMainMenuIntermediate($from, $newLang);
            }
            return;
        ...
        // Global Apply Now Route Execution Handlers
        if ($replyId === 'menu_apply') {
            ...
            if ($lang === 'en') {
                $text = "Thank you for your interest in Harvest International Schools!\n\nPlease apply directly through our Admissions Portal:\n👉 {$link}";
            } else {
                $text = "شكراً لاهتمامكم بمدارس هارڤست الدولية!\n\nيرجى تقديم طلب الالتحاق مباشرة عبر بوابة التقديم الالكترونية هنا:\n👉 {$link}";
            }
            sendFinalTextWithMenuButton($from, $text, $lang, 'main');
            return;
        }
        ...
        // Department Selection Menu Route for Direct WhatsApp Support Links
        if ($replyId === 'menu_chat') {
            sendChatDepartmentMenuIntermediate($from, $lang);
            return;
        }

        if (strpos($replyId, 'chat_dept_') === 0) {
            $deptId = str_replace('chat_dept_', '', $replyId);
            $dept = $config['contact_departments'][$deptId] ?? null;
            if ($dept) {
                $deptName = $dept[$lang];
                $waLink = "https://wa.me/" . $dept['number'];
                if ($lang === 'en') {
                    $text = "Please click the link below to open a direct chat with *{$deptName}*:\n\n👉 {$waLink}";
                } else {
                    $text = "يرجى الضغط على الرابط التالي لبدء محادثة مباشرة مع *{$deptName}*:\n\n👉 {$waLink}";
                }
                sendFinalTextWithMenuButton($from, $text, $lang, 'chat');
            }
            return;
        }

        // Static Content Handlers (Discounts, Accreditations, Careers)
        if (strpos($replyId, 'menu_') === 0) {
            if (in_array($replyId, ['menu_disc', 'menu_accr', 'menu_careers'])) {
                $text = $config['static_content'][$replyId][$lang];
        ...
        if (strpos($replyId, 'faq_') === 0) {
            ...
            if ($faq) {
                $text = "*" . $faq['q'][$lang] . "*\n\n" . $faq['a'][$lang];
                sendFinalTextWithMenuButton($from, $text, $lang, 'faq');
            }
            return;
        }

        // Department Selection Interceptor - Bypasses Stage Groups for Pre-Play/Play
        if (strpos($replyId, 'act_') === 0) {
            $parts = explode('_', $replyId);
            if (count($parts) >= 3) {
                $action = $parts[1];
        ...
        // Final Outcome Grade Selection Match Router
        if (strpos($replyId, 'res_') === 0) {
            $parts = explode('_', $replyId);
            $action = $parts[1];
            array_shift($parts);
            ...
            sendConsolidatedResult($from, $lang, $action, $stageId);
            return;
        }
    }
...
function sendFaqMenuIntermediate($to, $lang)
{
    ...
    $sections = [
        ...
        getNavSection($lang, 'faq')
    ];

    sendList($to, $ui['faq_body'][$lang], $ui['main_btn'][$lang], $sections);
}

function sendChatDepartmentMenuIntermediate($to, $lang)
{
    $config = getSchoolConfig();
    $ui = $config['ui'];

    $rows = [];
    foreach ($config['contact_departments'] as $id => $d) {
        $rows[] = ["id" => "chat_dept_" . $id, "title" => mb_substr($d[$lang], 0, 24)];
    }

    $sections = [
        [
            "title" => mb_substr($ui['chat_title'][$lang], 0, 24),
            "rows" => $rows
        ],
        getNavSection($lang, 'chat')
    ];

    sendList($to, $ui['chat_body'][$lang], $ui['main_btn'][$lang], $sections);
}
...
function sendDepartmentMenuIntermediate($to, $lang, $action)
{
    ...
    $sections = [
        ...
        getNavSection($lang, "dept_{$action}")
    ];
...
function sendSectionMenuIntermediate($to, $lang, $action, $deptKey)
{
    ...
    $sections = [
        ...
        getNavSection($lang, "sec_{$action}_{$deptKey}")
    ];
...
function sendStageMenuIntermediate($to, $lang, $action, $deptKey, $secKey)
{
    ...
    // Conditionally apply filters & text notices exclusively for tuition fees or admission requirements inquiries
    if ($action === 'fees' || $action === 'reqs') {
        $bodyText .= ($lang === 'en')
            ? "\n\n_Please note that unavailable stages will not be shown here._"
            : "\n\n_يرجى ملاحظة أن المراحل غير المتاحة لن تظهر هنا._";
    }

    foreach ($stageData['stages'] as $stageId => $stage) {
        if (($action === 'fees' || $action === 'reqs') && !$stage['offered']) {
            continue;
        }
        $id = "res_{$action}_{$stageId}";
        $rows[] = ["id" => $id, "title" => mb_substr($stage['name'][$lang], 0, 24)];
    ...
    $sections = [
        ...
        getNavSection($lang, "stg_{$action}_{$deptKey}_{$secKey}")
    ];

    sendList($to, $bodyText, $ui['main_btn'][$lang], $sections);
}

function sendFinalTextWithMenuButton($to, $text, $lang, $contextId = 'main')
{
    $config = getSchoolConfig();
    $ui = $config['ui'];
    $toggleTitle = ($lang === 'en') ? 'تغيير للغة العربية' : 'Change to English';
    ...
    sendButtons($to, $text, [
        ...
        ["id" => "lang_toggle_" . $contextId, "title" => mb_substr($toggleTitle, 0, 20)],
