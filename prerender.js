...
for (const url of routesToPrerender) {
    ...
    // --- NEW CODE: INLINE RENDER-BLOCKING CSS ---
    finalHtml = finalHtml.replace(
        /<link\s+rel="stylesheet"\s+[^>]*href="(\/assets\/[^"]+\.css)"[^>]*>/gi,
        (match, cssPath) => {
            try {
                // Read the actual CSS file generated by Vite
                const absoluteCssPath = toAbsolute(`dist/static${cssPath}`);
                ...
                return match; // Fallback to original <link> if file isn't found
            }
        }
    );
    // --- END NEW CODE ---
